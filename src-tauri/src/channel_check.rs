...
//! CSS (самый надёжный) -> MFSK -> OFDM+QPSK -> OFDM+16-QAM (самый быстрый из реальных).
//!
...
// Пробные тоны — в РАБОЧЕЙ полосе связи (~0.5–3.7 кГц, см. bandplan::Profile). Раньше зонд
// светил тонами до 12.5 кГц, которые реальное железо почти не воспроизводит, поэтому медианный
// SNR всегда занижался и зонд ошибочно советовал CSS. Теперь меряем ту полосу, где реально
// идёт передача, — оценка канала и рекомендация режима стали честными.
const TEST_FREQS_HZ: [f32; 8] = [
    600.0, 1000.0, 1400.0, 1800.0, 2300.0, 2800.0, 3200.0, 3600.0,
];
...
pub(crate) fn select_mode(snr_db: f32) -> (&'static str, &'static str, u32) {
    // Верхняя ступень — 16-QAM: это самый быстрый РЕАЛЬНО реализованный режим. 64-QAM
    // раньше рекомендовался тут, но модема под него нет (только план в PROTOCOL.md) —
    // не советуем то, что нельзя передать.
    if snr_db < 4.0 {
        ...
        ("OFDM_16QAM", "OFDM + 16-QAM — максимальная скорость", 20_000)
    }
}
