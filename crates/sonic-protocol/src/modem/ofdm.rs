...
impl Modem for OfdmModem {
    ...
    fn demodulate(&self, samples: &[f32]) -> Option<Demodulated> {
        ...
        let (d_coarse, cfo) = schmidl_cox(&bb)?;
        ...
        // 2b. ТОЧНЫЙ тайминг: matched-фильтр по известной преамбуле вокруг грубой оценки.
        // В узкой полосе метрика Schmidl-Cox размазана, поэтому без этого шага тайминг уезжал
        // и кадр не декодировался вчистую (хотя через шумный канал иногда везло).
        let d_peak = self.refine_timing(&bb, d_coarse)?;
...
impl OfdmModem {
    /// Точная временная синхронизация matched-фильтром: коррелируем принятый baseband с
    /// ИЗВЕСТНЫМ временным сигналом преамбулы вокруг грубой оценки Schmidl-Cox. В узкой полосе
    /// автокорреляционная метрика SC размазана и даёт ошибку в сотни сэмплов; взаимная
    /// корреляция с эталоном даёт острый пик точно на начале полезной части преамбулы.
    ///
    /// Преамбула — две одинаковые половины во времени, поэтому коррелируем КАЖДУЮ половину
    /// окна с эталоном первой половины и складываем МОДУЛИ: так оценка устойчива к остаточному
    /// CFO (фазовый набег в пределах половины мал). Возвращает начало полезной части преамбулы.
    fn refine_timing(&self, bb: &[Complex32], coarse: usize) -> Option<usize> {
        let pre = self.preamble_symbol(); // [CP][N]
        let href = &pre[CP..CP + HALF]; // эталон одной (первой) половины
        let ref_energy: f32 = href.iter().map(|c| c.norm_sqr()).sum::<f32>().max(1e-12);
        // Окно поиска ±N/2 вокруг грубой оценки — с запасом перекрывает ошибку SC.
        let lo = coarse.saturating_sub(N / 2);
        let hi = (coarse + N / 2).min(bb.len().saturating_sub(N));
        if hi <= lo {
            return None;
        }
        let mut best = 0.0f32;
        ...
        for start in lo..=hi {
            let mut a1 = Complex32::new(0.0, 0.0);
            let mut a2 = Complex32::new(0.0, 0.0);
            let mut e = 0.0f32;
            for (n, &rc) in href.iter().enumerate() {
                let x1 = bb[start + n];
                let x2 = bb[start + HALF + n];
                a1 += x1 * rc.conj();
                a2 += x2 * rc.conj();
                e += x1.norm_sqr() + x2.norm_sqr();
            }
            // Нормированная когерентность обеих половин: устойчива к общему уровню (AGC) и
            // к энергетической асимметрии, из-за которой «плыла» сырая метрика SC.
            let score = (a1.norm() + a2.norm()).powi(2) / (e.max(1e-12) * ref_energy);
            if score > best {
                best = score;
                best_start = start;
            }
...
fn schmidl_cox(bb: &[Complex32]) -> Option<(usize, f32)> {
    ...
    // Порог детекции 0.35 (снижен с идеальных 0.5): реальный захват (реверберация, AGC
    // микрофона, клиппинг динамика) роняет метрику. От ложных срабатываний на шуме защищает
    // энергетический гейт r_gate: в тишине/шуме R мало → метрика не проходит гейт.
    const THRESHOLD: f32 = 0.35;

    // Метрика Шмидла-Кокса даёт лишь ГРУБУЮ привязку: в узкой полосе (мало поднесущих) сигнал
    // самокоррелирован, метрика размазана — плато переходит в ложные пики уже на данных, и
    // глобальный argmax уезжал на сотни сэмплов (кадр не декодировался «вчистую», хотя через
    // шумный канал иногда везло). Точный тайминг добирает matched-фильтр по ИЗВЕСТНОЙ
    // преамбуле в `refine_timing`. Здесь достаточно ПЕРВОГО участка выше порога — это
    // преамбула (первая энергия после тишины), а не поздний ложный пик на данных.
    let onset = metrics
        .iter()
        .position(|&(m, rr, _)| rr >= r_gate && m >= THRESHOLD)?;
    // Дробный CFO по половинкам в районе преамбулы (грубо; остаток снимет пилот-трекинг).
    let cfo = metrics[onset].2.arg() / PI;
    Some((onset, cfo))
}
